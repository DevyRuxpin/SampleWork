...
from mock_services import mock_ai_service, mock_twilio_service
...
app = FastAPI(
    title="Phone Scheduler Bot (Mock Mode)",
    description="AI-powered phone call scheduling and management system - DEMO VERSION",
    version="1.0.0"
)
...
async def startup_event():
    ...
    try:
        logger.info("Starting Phone Scheduler Bot in MOCK MODE")
        logger.info("No real API keys required - using mock services")
        ...
        logger.info("Application started successfully in mock mode")
...
async def root():
    ...
    return {
        "message": "Phone Scheduler Bot is running in MOCK MODE",
        "version": "1.0.0",
        "status": "healthy",
        "mode": "mock",
        "note": "No real API keys required - all services are mocked"
    }
...
@app.get("/demo/sms")
async def demo_sms_endpoint():
    """Demo endpoint to simulate SMS interactions"""
    return {
        "message": "Demo SMS Endpoint",
        "instructions": "Use this endpoint to test SMS functionality",
        "example_requests": [
            "Schedule a call for tomorrow at 2pm",
            "What calls do I have scheduled?",
            "Cancel my upcoming call",
            "Reschedule my call to 3pm tomorrow"
        ],
        "endpoint": "/demo/sms/simulate"
    }

@app.post("/demo/sms/simulate")
async def simulate_sms(request: Request):
    """Simulate SMS processing without requiring Twilio webhook"""
    try:
        data = await request.json()
        user_phone = data.get("from", "+1234567890")
        message_body = data.get("body", "")
        
        if not message_body:
            raise HTTPException(status_code=400, detail="Message body is required")
        
        logger.info(f"📱 Simulating SMS from {user_phone}: {message_body}")
        
        # Process message immediately for demo
        await process_sms_message(user_phone, message_body)
...
async def get_demo_messages():
    """Get all sent mock messages for demo purposes"""
    messages = mock_twilio_service.get_sent_messages()
    return {
...
async def sms_webhook(request: Request):
    """Handle incoming SMS webhook from Twilio (mock mode)"""
    try:
        # Parse form data from Twilio
        form_data = await request.form()
        
        # Extract message details
        from_number = form_data.get("From", "+1234567890")
        to_number = form_data.get("To", "+0987654321")
...
async def process_sms_message(user_phone: str, message: str):
    """Process incoming SMS message using mock services"""
    try:
        # Get or create user
        ...
        ai_response = await mock_ai_service.process_message(
        ...
        if intent == "SCHEDULE_CALL":
            ...
            await mock_twilio_service.send_sms(user_phone, ai_response.get("response", ""))
            
    except Exception as e:
        logger.error(f"Error processing SMS message: {e}")
        # Send error message to user
        await mock_twilio_service.send_sms(
            user_phone, 
    ...
    try:
        ...
        if date_str and time_str:
            ...
            if result.get("success"):
                success_message = mock_ai_service.generate_success_message("SCHEDULE_CALL", extracted_data)
                await mock_twilio_service.send_sms(user_phone, success_message)
            else:
                error_message = f"Sorry, I couldn't schedule your call: {result.get('error')}"
                await mock_twilio_service.send_sms(user_phone, error_message)
        else:
            # Send confirmation request
            confirmation_message = mock_ai_service.generate_confirmation_message("SCHEDULE_CALL", extracted_data)
            await mock_twilio_service.send_sms(user_phone, confirmation_message)
            
    except Exception as e:
        logger.error(f"Error handling schedule call: {e}")
        await mock_twilio_service.send_sms(user_phone, "Sorry, there was an error scheduling your call. Please try again.")
...
async def handle_reschedule_call(user_phone: str, extracted_data: Dict[str, Any], ai_response: Dict[str, Any]):
    ...
    try:
        ...
        if not upcoming_calls:
            await mock_twilio_service.send_sms(user_phone, "You don't have any upcoming calls to reschedule.")
            return
        ...
        if date_str and time_str:
            ...
            if result.get("success"):
                success_message = mock_ai_service.generate_success_message("RESCHEDULE_CALL", extracted_data)
                await mock_twilio_service.send_sms(user_phone, success_message)
            else:
                error_message = f"Sorry, I couldn't reschedule your call: {result.get('error')}"
                await mock_twilio_service.send_sms(user_phone, error_message)
        else:
            confirmation_message = mock_ai_service.generate_confirmation_message("RESCHEDULE_CALL", extracted_data)
            await mock_twilio_service.send_sms(user_phone, confirmation_message)
            
    except Exception as e:
        logger.error(f"Error handling reschedule call: {e}")
        await mock_twilio_service.send_sms(user_phone, "Sorry, there was an error rescheduling your call. Please try again.")
...
async def handle_cancel_call(user_phone: str, ai_response: Dict[str, Any]):
    ...
    try:
        ...
        if not upcoming_calls:
            await mock_twilio_service.send_sms(user_phone, "You don't have any upcoming calls to cancel.")
            return
        ...
        if result.get("success"):
            success_message = mock_ai_service.generate_success_message("CANCEL_CALL", {})
            await mock_twilio_service.send_sms(user_phone, success_message)
        else:
        ...
        await mock_twilio_service.send_sms(user_phone, "Sorry, there was an error cancelling your call. Please try again.")
...
async def handle_check_calls(user_phone: str, ai_response: Dict[str, Any]):
    ...
    try:
        ...
        if not upcoming_calls:
            await mock_twilio_service.send_sms(user_phone, "You don't have any upcoming calls scheduled.")
            return
        ...
        await mock_twilio_service.send_sms(user_phone, call_info)
        
    except Exception as e:
        logger.error(f"Error handling check calls: {e}")
        await mock_twilio_service.send_sms(user_phone, "Sorry, there was an error checking your calls. Please try again.")
